...
# -*- coding: utf-8 -*-
"""
Created on Thu Apr  4 14:08:36 2024
...
def get_frame():
    ...
    while flag:
        ...
        if frame.shape==(720, 960, 3):   
            ...
            predicted_classes,im0=inference(frame,model,names,line_thickness=3)

            # image_with_faces, detected_faces = detect_faces(frame.copy())
            frame_rgb = im0
            battery = my_drone.get_battery()
            frame_rgb = cv2.putText(frame_rgb, f'Battery: {str(battery)} %', (760, 700), cv2.FONT_HERSHEY_SIMPLEX,  0.8, (255, 255, 255), 1, cv2.LINE_AA)
            frame_rgb = cv2.putText(frame_rgb, f'Command: {cmd}', (700, 670), cv2.FONT_HERSHEY_SIMPLEX,  0.8, (255, 255, 255), 1, cv2.LINE_AA)
